...
if (!isset($_GET['pg'])) {
	...
	if(count($rutas)>1 && is_numeric($rutas[1])){
		...
		$articulos = blog_controller::mostrar_busquedas_ctr($pagina_actual, $rutas[0]);
	}else if (count($rutas)>1 && !is_numeric($rutas[1])) {

	}else if(count($rutas)==1 && is_numeric($rutas[0])){
		$pagina_actual = $rutas[0];
		$articulos = blog_controller::mostrar_articulos_ctr($pagina_actual, null, null);
	}else if (count($rutas)==1 && !is_numeric($rutas[0])) {
		...
		if ($rut != "categorias"){
			...
			if (is_bool($articulo)) {
				$articulo = null;
				$rut = "buscador";
				$articulos = blog_controller::mostrar_busquedas_ctr(1, $rutas[0]);
				...
				$articulosCat = blog_controller::mostrar_articulos_ctr(null, "titulo", $categoria);
			}
		}else{
	...
	<?php
		...
		if(!isset($_GET['pg'])){
			...
			if(count($rutas)>1 && is_numeric($rutas[1])){
				...
				if ($rut == "categorias") {
					include_once 'pages/categorias.php';
				}else if($rut ==  "buscador"){
					include_once 'pages/resultados.php';
				}else if (isset($articulo)) {
